...
in {
    mapHosts' = hostsTypeDir: 
        let
            hostsNamesOfSystems = mapAttrs
                (system: _: 
                    (readDir "${toString hostsTypeDir}/${system}")
                )
                (readDir hostsTypeDir); # { hostTypeSystem: { hostName: "directory", ... }, ... }

            systemOfHosts = lib.foldl' 
                (hostsSystem: system:
                    let 
                        hostsNames = hostsNamesOfSystems.${system}; # { hostName: "directory", ... }
                    in hostsSystem // lib.foldl'
                        (hostsSystem: hostName: 
                            hostsSystem // {${hostName} = { inherit system; };})
                        {}
    ...
    mapHostsOfType = systemOfHosts: mkHost: hostsTypeDir: mapAttrs
        (hostName: hostInfo: 
            mkHost
                "${toString hostsTypeDir}/${hostInfo.system}/${hostName}"
                hostInfo.system
        )
        systemOfHosts;
